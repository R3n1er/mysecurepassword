...
describe('Positionnement Google Workspace', () => {
  ...
  it('devrait avoir la checkbox Google Workspace avec la bonne description', () => {
    ...
    // Vérifier le texte descriptif
    expect(screen.getByText('Évite les caractères ambigus (l, 1, I, O, 0)')).toBeInTheDocument();
    
    // Vérifier que la checkbox est présente
    const checkbox = screen.getByRole('checkbox', { name: /Mode Google Workspace/i });
  ...
  it('devrait avoir une structure logique de configuration', () => {
    ...
    const container = document.querySelector('.msp-section-generator');
    const configSection = container?.querySelector('h3')?.parentElement?.parentElement;
    
    // Vérifier que la section Configuration existe
    expect(screen.getByText('Configuration')).toBeInTheDocument();
    expect(screen.getByText('Personnalisez votre mot de passe')).toBeInTheDocument();
