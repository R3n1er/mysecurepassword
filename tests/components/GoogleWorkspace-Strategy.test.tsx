...
describe("Stratégie Google Workspace - Tests Optimisés 2024", () => {
  ...
  describe("Interface utilisateur - Mode Google Workspace", () => {
    it("devrait afficher la description optimisée pour Google Workspace", () => {
      ...
      // La description peut varier, on vérifie des fragments robustes
      // Vérifier la présence d'une des formulations possibles
      const fragments = [/caractères similaires/i, /non compatibles/i];
      const found = fragments.some((f) => !!screen.queryByText(f));
    ...
    it("devrait adapter la description des symboles selon le mode", () => {
      ...
      // En mode standard, la description liste tous les symboles, dont /~`
